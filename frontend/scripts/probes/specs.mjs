...
const m = (b) => b.eval(`(()=>{const rows=document.querySelectorAll('.specs-table tbody tr'); const card=document.querySelector('.specs-card'); const body=document.querySelector('.specs-card__body'); return {viewport:[innerWidth,innerHeight], rows:rows.length, rowH:rows[0]?Math.round(rows[0].getBoundingClientRect().height):null, card:!!card, stages:document.querySelectorAll('.specs-card .stage-cell').length, tabs:document.querySelectorAll('.specs-card__tabs .panel__tab').length, normsScroll:(e=>e?e.scrollHeight-e.clientHeight:null)(document.querySelector('.specs-card__norms')), costScroll:(e=>e?e.scrollHeight-e.clientHeight:null)(document.querySelector('.specs-card__cost')), pageScroll:document.documentElement.scrollHeight-innerHeight, tableOverflow:(e=>e?e.scrollWidth-e.clientWidth:null)(document.querySelector('.specs-table-wrap')), url:location.search};})()`);
export default async function (b) {
  ...
  for (const [w, h] of [[1440, 900], [1280, 800]]) {
    ...
    const tab = await b.rect('.specs-card__tabs .panel__tab', 1); await b.click(tab.cx, tab.cy); await b.wait(900);
    out[`bom${w}`] = await b.eval(`({view:document.querySelector('.specs-card__tabs [aria-selected="true"]')?.textContent.slice(0,14), rows:document.querySelectorAll('.specs-card__view .bom-row, .specs-card__view tbody tr').length, viewH:Math.round(document.querySelector('.specs-card__view').clientHeight), viewScroll:(e=>e.scrollHeight-e.clientHeight)(document.querySelector('.specs-card__view')), pageScroll:document.documentElement.scrollHeight-innerHeight})`);
