...
import {
  Card, Stack, Group, Text, Badge, SimpleGrid, Skeleton, Table, Box, Progress, Alert,
} from '@mantine/core';
            ...
            <Group gap="lg" mb="md" wrap="wrap">
              ...
              {line.upcomingTotal > 0 && (
                <Group gap={6}>
                  ...
                  <Badge variant="light" color="gray" radius="xl">{formatCurrency(line.upcomingTotal)}</Badge>
                </Group>
              )}
                  ...
                  <Table.Tbody>
                    {line.recentPayments.map((p, i) => (
                      <Table.Tr key={i}>
                        ...
                        <Table.Td ta="right" ff="monospace">{formatCurrency(p.totalAmount)}</Table.Td>
                        <Table.Td ta="right" ff="monospace" c="dimmed">{formatCurrency(p.principalAmount)}</Table.Td>
                        <Table.Td ta="right" ff="monospace" c="dimmed">{formatCurrency(p.interestAmount)}</Table.Td>
                        <Table.Td>
                          <Badge size="xs" variant="light" color={p.status.toLowerCase().includes('оплач') ? 'success' : 'gray'}>
                            {p.status}
                          </Badge>
                        </Table.Td>
                      </Table.Tr>
