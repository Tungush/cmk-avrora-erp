...
/**
 * ИЗВЕСТНОЕ ОГРАНИЧЕНИЕ РЕЖИМА ДИЗАЙНА (04.09.2026, найдено прогоном /qa).
 *
 * Отметка «Изготовлено» в цеху показывает сообщение об успехе, но список
 * и счётчики не меняются. Это НЕ баг продукта: на живом бэкенде мутация
 * работает, и фронтенд корректно сбрасывает кэш запроса ['shop-floor'].
 *
 * Причина в самих фикстурах. Модули orders.ts и production.ts — два
 * НЕЗАВИСИМЫХ генератора с разными функциями идентификаторов: в заказах
 * `uuid()` без аргументов, в производстве `uuid(...parts)` от содержимого.
 * Один и тот же заказ получает в них разные идентификаторы, поэтому
 * PATCH /orders/:id/production-stages/:code пишет в копию заказов, а
 * список цеха читает копию производства и записи не находит.
 *
 * Накладкой поверх это не лечится — я пробовал: ключи всё равно из разных
 * вселенных. Настоящее лечение — общий генератор идентификаторов для
 * обоих модулей, то есть переделка каркаса фикстур. Отдельная работа.
 *
 * Пока помните при проверке цеха: сообщение об успехе там правдиво,
 * неизменившийся счётчик — артефакт фикстур, а не продукта.
 */
