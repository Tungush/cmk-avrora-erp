...
// RateForStage — ставка для передела: участок → передел → общая. Участок
// (workCenterRate) остаётся самым точным уровнем — он про конкретную
...
func RateForStage(stage Stage, rates CostingRates, workCenterRate *float64) float64 {
	if workCenterRate != nil {
		return *workCenterRate
	}
